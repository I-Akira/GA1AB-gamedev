...
引数3 RECT_F*  dst  :描写位置
引数4 float    c[]/ :カラー情報
ブロックを64*64限定描画用。リソース切り取り位置のみでx,yで
設定できる
*/
...
//Block関数
//引数1	 float* x			:行うobjectのX位置
//引数2	 float* y			:判定を行うobjectのY位置
//引数3  bool	scrolll_on	:判定を行うobjectはスクロールの影響与えるかどうか(true=与える false=与えない)
//引数4　bool*	up			:上下左右判定の上部分に当たっているかどうかを返す
//引数5  bool*	down		:上下左右判定の下部分に当たっているかどうかを返す
//引数6  bool*	left		:上下左右判定の下部分に当たっているかどうかを返す
//引数7  bool*	right		:上下左右判定の下部分に当たっているかどうかを返す
...
#define SGN(x) 1 - (x <= 0 ) - (x < 0 )
	...
	引数 float 3, 4  a2x , a2y		;軸ベクトルBの始点
	...
	主人公の位置+移動ベクトルとかくブロックの辺で交点判定を行い
	最も近い交点の位置と距離を返す*/
